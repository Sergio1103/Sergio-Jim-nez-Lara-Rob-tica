...
# Animación del desplazamiento en X 
def animate_shift(steps=20, shift=10):
    """
    Esta función anima el desplazamiento de la caja en el eje X.
    steps → número de pasos de la animación (controla suavidad/velocidad).
    shift → cuánto se mueve en total la caja en X.
    """
    ...
    while n <= steps:
        ...
        # Desplazamiento proporcional en X
        # (shift/steps) es cuánto avanza la caja por cada paso
        dx = (shift/steps)*n

        # Aplicar desplazamiento a cada punto de la caja
        p1 = p1_init + [dx,0,0]
        p2 = p2_init + [dx,0,0]
        p3 = p3_init + [dx,0,0]
        p4 = p4_init + [dx,0,0]
        ...
        # Dibuja la caja en su nueva posición desplazada
...
# la caja se moverá 15 unidades en X en 30 pasos
animate_shift(steps=30, shift=15)
